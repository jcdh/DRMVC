...
class VAE(nn.Module):
    def __init__(self, img_size, latent_spec, temperature=.67,
                 ...
                 hidden_dim=256, shareAE=1, input_size=1, dimss=[], min_k=20):
        """
        ...
        if self.Net == 'C':
            ...
            self.mu = nn.Linear(self.hidden_dim, self.latent_cont_dim)
            self.k = nn.Linear(self.hidden_dim, 1)
    ...
    def encode(self, X):
        ...
        if self.is_continuous:
            for i in range(self.view_num):
                continue_name = 'cont' + str(i+1)
                mu = self.mu(hiddens[i])

                # normalization
                mu = mu / mu.norm(dim=1, keepdim=True)
                kappa_k = F.softplus(self.k(hiddens[i])) + self.min_k
                latent_dist[continue_name] = [mu, kappa_k]
    ...
    def reparameterize(self, latent_dist):
        ...
        if self.is_continuous:
            for i in range(self.view_num):
                ...
                mu, k = latent_dist[countinus_name]
                q_z = VonMisesFisher(mu, k)
                z = q_z.rsample()
                latent_sample.append(z)
