...
class Trainer():
    def __init__(self, model, optimizer, model_name, cont_capacity=None,
        ...
        **Acknowledgments**
        ...
        self.loss_r = [[] for _ in range(self.view_num)]  # recon_loss
        self.loss_z = [[] for _ in range(self.view_num)]  # kl_loss z
        self.loss_c = [[] for _ in range(self.view_num)]  # kl_loss c
    ...
    def train(self, data_loader, net, epochs=10, save_training_gif=None):
        """
        Trains the model.
        ...
        for epoch in range(epochs):
            mean_epoch_loss = self._train_epoch(data_loader, net)
            for i in range(self.view_num):
        ...
        mean_loss_file_name = './mean_loss' + self.model_name + '.npy'
        np.save(mean_loss_file_name, self.mean_loss)
    ...
    def _train_epoch(self, data_loader, net):
        """
        ...
        for batch_idx, Data in enumerate(data_loader):
            iter_loss = self._train_iteration(Data[0:-1], net)
            # print(iter_loss)
            for i in range(self.view_num):
    ...
    def _train_iteration(self, data, net):
        """
        ...
        constrastive_Loss = 0
        # Cross-view contrastive loss: iterate over each view and compute loss between a randomly chosen pair of views
        for u in range(self.view_num):
            possible_values = [i for i in range(self.view_num) if i != u]
            v = random.choice(possible_values)
            # Create view labels for the two views selected
            view_1 = 'cont' + str(u + 1)
            view_2 = 'cont' + str(v + 1)
            # Calculate cross-view contrastive loss between view `u` and view `v` and accumulate it
            constrastive_Loss += self._crossview_contrastive_Loss(latent_dist[view_1], latent_dist[view_2], self.view_num)
        # Append the cross-view contrastive loss to the loss list
        Loss.append(constrastive_Loss)
    ...
    def _loss_function(self, data, recon_data, latent_dist, view=0, max_recon_loss=1000, beta=1, latent_cont_dim=10):
        """
        ...
        total_loss = recon_loss + disc_capacity_loss
    ...
    """
        code from https://github.com/ssnl/align_uniform/blob/master/align_uniform/__init__.py
        paper link: https://arxiv.org/abs/2005.10242
    """
    ...
    # Function to calculate cross-view contrastive loss between two specific views
    def _crossview_contrastive_Loss(self, view1_cont, view2_cont, view_num):

        mu1, k1 = view1_cont
        mu2, k2 = view2_cont
        view1 = VonMisesFisher(mu1, k1).rsample()
        view2 = VonMisesFisher(mu2, k2).rsample()
        align_loss_val = self.align_loss(view1, view2, alpha=2)
        unif_loss_val = (self.uniform_loss(view1, t=2) + self.uniform_loss(view2, t=2)) / 2*(view_num-1)
        loss = align_loss_val + unif_loss_val
        return loss